...
exports.getExpenses = (req, res) => {
  ...
  let sql = `SELECT e.*, c.name as category_name, c.color 
             FROM expenses e 
...
exports.createExpense = (req, res) => {
  ...
  if (amount <= 0) return res.status(400).json({ message: 'Le montant doit être > 0' });
...
exports.updateExpense = (req, res) => {
  ...
  db.run(`UPDATE expenses SET amount=?, date=?, note=?, category_id=? 
